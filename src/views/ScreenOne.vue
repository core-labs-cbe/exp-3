...
export default {
  setup() {
    ...
    const onDrop = (event, list) => {
     ...
     if((items.value[idx].list === 6) && (items.value[idx].list === 8) && (items.value[idx].list === 10)  && (items.value[idx].list === 12) && (items.value[idx].list === 13)){
  ...
  methods: {
    ...
    validate() {
      ...
      {
        if (customer.includes("customerName String") && (customer.length==1)) {
          this.valid[0] = true;
        }

        if (clerk.includes("clerkName String") && (clerk.length==1)) {
          this.valid[1] = true;

        }

        if (vehicle.includes("regNum int") && vehicle.includes("keyStatus String") && (vehicle.length==2)) {
          this.valid[2] = true;
        }

        if (reservation.includes("customerObj Customer") && reservation.includes("vehicleObj Vehicle") && reservation.includes("contractStatus String") && reservation.includes("releaseDate Date") && (reservation.length==4)) {
          this.valid[3] = true;
        }

        if((this.valid[0] == true) && (this.valid[1] == true) && (this.valid[2] == true) && (this.valid[3] === true)){
...

      {
        ...
        setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
          ...
          var x = document.getElementById("snackbar");
        x.innerHTML = "There is a wrong attribute entry / type mismatch / missing entry in Customer";
        x.style.backgroundColor = "red";
        x.className = "show";
        setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
        }
          ...
          var x = document.getElementById("snackbar");
        x.innerHTML = "There is a wrong attribute entry / type mismatch / missing entry in Clerk";
        x.style.backgroundColor = "red";
        x.className = "show";
        setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
        }
          ...
          var x = document.getElementById("snackbar");
        x.innerHTML = "There is a wrong attribute entry / type mismatch / missing entry in Vehicle";
        x.style.backgroundColor = "red";
        x.className = "show";
        setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
        }
          ...
          var x = document.getElementById("snackbar");
        x.innerHTML = "There is a wrong attribute entry / type mismatch / missing entry in Reservation";
        x.style.backgroundColor = "red";
        x.className = "show";
        setTimeout(function () { x.className = x.className.replace("show", ""); }, 3000);
        }
...
<template>
  <header style="padding: 20px">
    ...
    <p>Map the elements and respective datatypes to their classes by dragging and dropping. Make sure you're dragging variables and the datatypes at the same time</p>
  ...
  <main>
    ...
    <div class="flexbox">
      <div class="flex-between sticky">
        <h3>Variables</h3>
        <div class="identify-box drop-zone" @dragenter.prevent @dragover.prevent>
